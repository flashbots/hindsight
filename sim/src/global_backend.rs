...
use revm::{
    db::{CacheDB, EmptyDB},
    precompile::Bytes as pBytes,
    primitives::{
        AccountInfo, Address as rAddress, Bytecode, Bytes, B256, KECCAK_EMPTY, U256 as rU256,
    },
};
use std::{
...
// use super::db_error::{DatabaseError, DatabaseResult};
...
type AccountInfoSender = OneshotSender<Result<AccountInfo>>;
type StorageSender = OneshotSender<Result<rU256>>;
type BlockHashSender = OneshotSender<Result<B256>>;

type BasicFuture<Err> =
    Pin<Box<dyn Future<Output = (Result<(rU256, u64, Bytes), Err>, rAddress)> + Send>>;
        ...
        match self.account_requests.entry(address) {
            ...
            Entry::Vacant(entry) => {
                ...
                let fut = Box::pin(async move {
                    ...
                    let address_ethers: Address = (address.0).0.into();
    ...
    fn request_hash(&mut self, number: rU256, listener: BlockHashSender) {
        match self.block_requests.entry(number) {
            ...
            Entry::Vacant(entry) => {
                ...
                let fut = Box::pin(async move {
                    // convert from revm to ethers type
                    let number_ethers: u64 = U256::from(number.to_be_bytes()).as_u64();
                    ...
                    FetchRequestFuture::Basic(fut) => {
                        if let Poll::Ready((resp, addr)) = fut.poll_unpin(cx) {
                            ...
                            let (code, code_hash) = if !code.is_empty() {
                                ...
                                (Some(pBytes::default()), KECCAK_EMPTY)
                            };
